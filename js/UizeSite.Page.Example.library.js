...
Uize.module({name:'UizeSite.Page.Example',required:['Uize.Node','Uize.Url'],builder:function(e_a){var e_b=e_a.subclass(),e_c=e_b.prototype;e_c.wireUi=function(){var e_d=this;if(!e_d.isWired){e_d.injectNodeHtml('actions','<a id="'+e_d.get('idPrefix')+'-delve" href="javascript://" class="buttonLink" title="Launch the DELVE tool to inspect the inner workings of this page">DELVE</a>','inner bottom');e_d.wireNode('delve','click',function(){var e_e=window.screen,e_f=e_e.width-50,e_g=e_e.height-100,e_h=['<html>',
'<head><title>DELVE</title></head>','<body>','<script src="http://www.uize.com/js/Uize.js"></script>','<script type="text/javascript">','Uize.module ({','required:\'UizeSite.DelvePageWriter\',','builder:function () {','UizeSite.DelvePageWriter.initialize ();','}','});','</script>','</body>','</html>'].join('\n'),e_i=window.open('javascript:\''+e_h.replace(/'/g,'\\\'')+'\'','reportPopup',['width='+e_f,'height='+e_g,'left='+Math.max((e_e.width-e_f-10)>>1,0),'top='+Math.max((e_e.height-e_g-40)>>1,0),'toolbar=no','location=no','directories=no','status=no','menubar=no','resizable=yes','scrollbars=no'].join(','));e_i.focus();});e_d.e_j&&e_d.wireNode(Uize.Node.find({tagName:'A',className:/\blinkedJs\b/}),'click',function(){e_d.e_j(this.title||this.innerHTML)});e_a.prototype.wireUi.call(e_d);var e_k=Uize.Url.fromParams(location.href).tour;e_k&&Uize.module({required:['UizeSite.Templates.Tour','Uize.Tooltip','Uize.Url','UizeSite.Examples'],builder:function(){Uize.Node.injectHtml(document.body,
UizeSite.Templates.Tour.process({tour:e_k,pageUrl:location.href}));function e_l(e_m){var e_n=e_l.e_o;if(!e_n){e_n=e_l.e_o={};Uize.forEach(UizeSite.Examples(),function(e_p){e_n[Uize.Url.from(e_p.path).fileName]=e_p;});}return e_n[Uize.Url.from(e_m).fileName];}e_d.wireNode(Uize.Node.find({tagName:'a',className:/\b(tourPage|tourButton)\b/}),{mouseover:function(){var e_p=e_l(this.getAttribute('href'));e_d.setNodeValue('tourPageTooltip-title',e_p.title);e_d.setNodeValue('tourPageTooltip-description',e_p.description);e_d.setNodeValue('tourPageTooltip-keywords',e_p.keywords||'-- NONE --');Uize.Tooltip.showTooltip('page-tourPageTooltip');},mouseout:function(){Uize.Tooltip.showTooltip('page-tourPageTooltip',false)}});}});}};e_b.registerProperties({e_j:'evaluator'});e_b.set({showFooter:false});return e_b;}});
