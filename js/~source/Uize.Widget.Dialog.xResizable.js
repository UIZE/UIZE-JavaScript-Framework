				...
				if (_this.isWired && _this.get('shown') && !_this.get('inDrag')) {
					if (!_this.get('isMaximized')) 
						...
						_this.setNodeStyle(
							_nodeToSetDimension,
							{
								//Needs a 3 px buffer so that overflow isn't added
								width: document.documentElement.clientWidth - (_rootDims.width - _contentDims.width) - 3,
								height: document.documentElement.clientHeight - (_rootDims.height - _contentDims.height) - 3
							}
						);
					}
				}
