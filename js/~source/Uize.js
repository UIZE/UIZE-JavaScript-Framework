...
(function () {
	...
	/*** Functions required by the Class Constructor ***/
		...
		function _copyInto (_targetObject,_sourceObject) {
			...
			_class.registerProperties = function (_propertyProfiles) {
				...
				for (var _propertyPrivateName in _propertyProfiles) {
					...
					if (_propertyPublicName.indexOf ('|') > -1) {
						...
						_class.lookup (_pseudonyms,_propertyProfile,_propertyProfilesByPublicNames);
					} else {
	...
	/*** Public Static Methods ***/
		...
		var _forEach = _nonInheritableStatic (
			'forEach',
			function (_source,_iterationHandler,_context,_allArrayElements) {
				if (_source) {
		...
		var _map = _nonInheritableStatic (
			'map',
		...
		_nonInheritableStatic ('isObject',_isObject);
			/*?
				Static Methods
					Uize.isObject
		...
		_nonInheritableStatic ('isArray',_isArray);
			/*?
				Static Methods
					Uize.isArray
