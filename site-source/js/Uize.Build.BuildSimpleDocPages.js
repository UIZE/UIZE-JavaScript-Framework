				...
				/*** extract SIMPLE doc from JavaScript modules ***/
					var
						...
						_modulesTree = Uize.Data.PathsTree.fromList (
							Uize.Wsh.getFiles (_moduleFolderPath,_dotJsRegExp,_getFilenameFromPath)
						),
						_fullModuleFolderPath = _scriptFolderPath + '\\' + _moduleFolderPath
					;
					...
					Uize.Wsh.buildFiles ({
						...
						logFileName:_params.logFileName
							...
							: '',
						targetFolderPathCreator:function (_folderPath) {
							var _referenceFolderPath = _folderPath == _fullModuleFolderPath
								? _scriptFolderPath + '\\' + _moduleReferenceFolderName
								: null
							;
							if (_referenceFolderPath)
								Uize.Wsh.fileExists (_simpleDocTemplatePath = _referenceFolderPath + '\\' + _simpleDocTemplateFileName)
									? (_simpleDocTemplate = null)
									: (_referenceFolderPath = null)
								;
							;
							return (
								_referenceFolderPath &&
								Uize.Build.Util.resolveBuiltFolderPath (_referenceFolderPath,_buildFolderPath)
							);
						},
						targetFilenameCreator:function (_sourceFileName) {
