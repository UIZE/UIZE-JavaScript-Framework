	...
	builder:function (_superclass) {
					...
					function () {
						...
						/*** set up the fade instance ***/
							...
							m.fade.wire ({
								...
								Done:
									function () {m._showPalette (m._shown)}
							});
							/*?
								Instance Properties
									fade
						...
						/*** create the selector button ***/
							var
								_selector = m._selector = m._addChildButton (
									'selector',
							...
							m._updateSelectorSelectedState ();
		...
		/*** Private Instance Methods ***/
			...
			_classPrototype._setDismissTimeout = function () {
				var
					m = this,
					_hideTimeout = !Uize.isNumber(m._hideWhenOut)
						? 250
						: m._hideWhenOut
				;

				m._clearDismissTimeout ();
					...
					/*** capture mousedown event for palette ***/
						m.wireNode (
							'palette',
							{
								...
								mouseover:function () {m._over ()},
								mouseout:function () {m._out ()}
							}
						);
			...
			_class.stateProperties ({
				...
				_shown:{
					...
					onChange:function () {
						...
						;
						m._selector && m._updateSelectorSelectedState ();
						if (m.isWired) {
							m._clearDismissTimeout ();
							if (_shown) {
