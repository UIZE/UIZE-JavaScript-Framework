	...
	builder:function () {
		...
		/*** Utility Functions ***/
			var
				...
				_getById = function(_nodeBlob) { return _Uize_Node.getById(_nodeBlob) },
				...
				_getSacredNode = function() { return _object._sacredNode = _object._sacredNode || document.createElement('DIV') },
				_isMatch = function(_node, _selector) {
					...
					return (
						_selector === _undefined
							...
							|| (_Uize_Node_isNode(_selector) && _node == _selector)
					);
				},
				...
				_select = function(_param, _rootNode) {
					...
					else if (_Uize_isArray(_param) && (!_param.length || _Uize_Node_isNode(_param[0]))) { // node list
						_nodes = [];
						for (var _nodeNo = -1; ++_nodeNo < _param.length;) _nodes[_nodeNo] = _param[_nodeNo];  // make a copy to be safe (can't use concat because may not be a native JS array)
				...
				_object = Uize.noNew (
					function () {
						var
							...
							_nodes = _this._nodes = _selectInfo.nodes,

							_nodesLength = _this.length = _nodes.length, // add public length property
							_nodeNo = -1
						;
						
						_this.supportsTouch = _object.supportsTouch;
						...
						/* call constructor hook method for extensions */
							_this.atEndOfConstructor(_selectInfo);
						...
						/*?
							...
							Instance Properties
								...
								supportsTouch
									...
									- See also =supportsUize.Web.supportsTouchTouch= static property
						*/
					}
				),
		...
		/*** Public Static Methods ***/
			...
			_object.selectCss = function(_selector, _rootNode) {
				var
					_domElement = _rootNode ? _getById(_rootNode) : document,
		...
		/*** Private Instance Methods ***/
			...
			_objectPrototype._handleGetAction = function(_getFunc, _returnAll) {
				var
					_nodes = this._nodes,
					_returnValue = _null
				;
			...
			_objectPrototype._handleGetOrSetAction = function(_getFunc, _returnAllOrValueToSet, _setFunc) {
				return (
			...
			/** General **/
				...
				_objectPrototype.element = function(_index) {
					...
					/*?
						Instance Methods
							element
								Retrieves the DOM nodes within the set of matched DOM nodes at the specified index.
					...
					/*?
						Instance Methods
							unwire
								...
								- See also the =unwire= static method in =Uize.Node=
					*/
				};
					...
					/*?
						Instance Methods
							wire
								...
								- See also the =wire= static method in =Uize.Node=
								- Calls =Uize.Node.fix= on the event object
					*/
				};
					...
					/*?
						Instance Methods
							wireOnce
								...
								- See also the =wire= and =unwire= static method in =Uize.Node=
					*/
				};
						...
						/*?
							Instance Methods
								altClick
									...
									- See the =Uize.Node.VirtualEvent= module for more information on virtual events
						*/
			...
			/** Style/CSS **/
				...
				_makeNodeGetSetMethod(
					...
					function (_node, _properties) {
						return _Uize_Node_getStyle(
							_node,
							_Uize_isString(_properties)
								? _supportsCss(_properties, _node)
								: _getSupportedProperties(_properties, _node)
						);
					},
					function (_node, _properties) { _Uize_Node_setStyle(_node, _getSupportedProperties(_properties, _node)) }
				);
					/*?
						Instance Methods
							css
				...
				var
					_makeCssMethod = function(_methodName, _propertyName) {
						_objectPrototype[_methodName] = function(_propertyValue) {
							var
								_this = this,
								_returnValue = _this.css(_propertyName, _propertyValue)
							;
				...
				/** Display-related methods **/
					...
					_objectPrototype.visibility = function(_mustShow) {
						this._handleSetAction(_mustShow, _Uize_Node.show);
						/*?
							Instance Methods
								show
					...
					return this._handleGetOrSetAction(
						function(_node) {
							...
							return {// optionally make coordinates window-relative
								left:_coords.left - (_param1 ? _window.pageXOffset : 0),
								top:_coords.top - (_param1 ? _window.pageYOffset : 0)
							};
						},
				...
				_objectPrototype.text = function(_returnAllOrValueToSet) {
					return this._handleGetOrSetAction(
						...
						function(_node, _text) {
							_node.appendChild(document.createTextNode(_text + '')); // coerce to a string value by invoking valueOf method
						}
					);
					/*?
						Instance Methods
